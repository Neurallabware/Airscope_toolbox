...
try:
    from cellpose import transforms as _cellpose_transforms
except Exception:
    _cellpose_transforms = None


def _normalize_img_local(img: np.ndarray, sharpen_radius: float = 3.0) -> np.ndarray:
    """Fallback normalization when `cellpose` is unavailable."""
    arr = img.astype(np.float32, copy=False)
    base = arr[..., 0] if arr.ndim == 3 else arr
    sigma = max(float(sharpen_radius), 0.5)
    blur = cv2.GaussianBlur(base, (0, 0), sigmaX=sigma, sigmaY=sigma)
    high = base - blur
    p1, p99 = np.percentile(high, [1, 99])
    denom = max(float(p99 - p1), 1e-6)
    norm = np.clip((high - p1) / denom, 0.0, 1.0)
    if arr.ndim == 3:
        norm = norm[..., np.newaxis]
    return norm


def _normalize_img(img: np.ndarray, sharpen_radius: float = 3.0) -> np.ndarray:
    if _cellpose_transforms is not None:
        return _cellpose_transforms.normalize_img(img, sharpen_radius=sharpen_radius)
    return _normalize_img_local(img, sharpen_radius=sharpen_radius)
...
def highpass_mean_image(I, aspect=1.):
    ...
    img_filt = _normalize_img(img_filt[..., np.newaxis], sharpen_radius=3)
